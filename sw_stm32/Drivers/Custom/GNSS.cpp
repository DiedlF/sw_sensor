...
GNSS_Result GNSS_type::update_delta(const uint8_t * data)
{
	...
	coordinates.relPosAccN   = p.accN * 0.0001f;           // 0.1mm -> m
	coordinates.relPosAccE   = p.accE * 0.0001f;
	coordinates.relPosAccD   = p.accD * 0.0001f;
	coordinates.relPosAccLen = p.acc_len * 0.0001f;
	coordinates.relPosHeadingAcc = p.acc_heading * 1.745329252e-7f; // 1e-5 deg -> rad
